...
import { NavGroupPermissionGuard } from 'src/components/guard/nav-group-permission-guard';
...
function Group({ items, render, subheader, slotProps, enabledRootRedirect }: NavGroupProps) {
  ...
  const permissionRequired = items.map((i) => i.permission)
  ...
  return (
    <NavGroupPermissionGuard requiredPermissions={permissionRequired}>
      <NavLi>
        {subheader ? (
          <>
            <Subheader
